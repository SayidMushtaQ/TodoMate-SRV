...
export const userLogin = asyncHanlder(async(req,res)=>{
    ...
    if([data,password].some(val=> val === '')){
        throw new ApiError(400,'Email or userName ans password is required',['Please fill up all necessary fields'])
    }
    let userName,email;
    ...
    return  res.status(200).json(
        new ApiResponse(200,{user:isUserExist,redirectURL:'/dashboard'},"Login successful.")
    )
})